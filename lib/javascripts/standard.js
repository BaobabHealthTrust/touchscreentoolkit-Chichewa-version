...
function createButtons() {
    ...
    buttonsDiv.innerHTML += "<button id='nextButton' class='button green navButton' onMouseDown='gotoNextPage()'><span>Patsogolo</span></button>";

    // create back button
    buttonsDiv.innerHTML += "<button id='backButton' class='button blue navButton'><span>Bwerelani</span></button>";

    // create clear button or new patient button if on search page
    if (!tstSearchPage) {
        buttonsDiv.innerHTML += "<button id='clearButton' class='button blue navButton' onMouseDown='clearInput()'><span>Yeretsani</span></button>";
    } else {
    ...
    buttonsDiv.innerHTML += "<button class='button navButton red' id='cancelButton' onMouseDown='confirmCancelEntry(" + (typeof(save_state) != "undefined"?"true":"") + ");'><span>Lekani</span></button>";
    ...
    if (!tstSearchPage) {
        if(tstFormElements[i].getAttribute("ajaxURL") != null){
            ...
            if(tstFormElements[i].tagName == "SELECT") {
                ...
                if (tstFormElements[i].checked) tstInputTarget.value = "Eya";
                else tstInputTarget.value = "Ayi";
            } else {
...
function navigateToPage(destPage, validate, navback){    
    ...
    if(destPage < tstPages.length){
        ...
        if (destPage+1 == tstPages.length) {
            nextButton.innerHTML = "<span>Malizani</span>";
        } else {
...
function inputIsValid() {
    ...
    if (validateResult.length > 0 && !tstSearchPage) {
        ...
        if (ttInput.shouldConfirm) {
            ...
            "gotoPage(tstCurrentPage+1, false);'><span>Eya</span></button><button class='button' " +
            "style='float: none; right: 3px;' onmousedown='this.offsetParent.style.display=\"none\"; '>" +
            "<span>Ayi</span></button>";
...
function confirmValue() {
    ...
    " onmousedown='validateConfirmUsername()'/><input type='submit' value='Lekani' " + 
...
function showMessage(aMessage, withCancel, timed) {
    ...
    "<br />" + (typeof(withCancel) != "undefined" ? (withCancel == true ?
        ...
        "clearTimeout(tstTimerHandle);'><span>Lekani</span></button>" : "") : "") +
...
function confirmCancelEntry(save) {     // If you want to save state set save =
    // true
    if (tstConfirmCancel) {
        tstMessageBar.innerHTML = "Mwatsimikizadi Mukufuna Kuleka?<br/>" +
...
function getQwertyKeyboard(){
    ...
    getButtonString('backspace','Fufutani') +
...
function dispatchMessage(message, messageBoxType) {
    ...
    switch(messageBoxType){
        case tstMessageBoxType.OKOnly:
            buttons = "<button class = 'button' onclick = 'this.offsetParent.style.display=\"none\"; gotoPage(tstCurrentPage+1, false);'> <span> Chabwino </span> </button>"
            break;

        case tstMessageBoxType.OKCancel:
            buttons = "<button class = 'button' onclick = 'this.offsetParent.style.display=\"none\"; gotoPage(tstCurrentPage+1, false);'> <span> Chabwino </span> </button>" +
            "<button class = 'button' onclick = 'this.offsetParent.style.display=\"none\";'> <span> Lekani </span> </button>"
            break;
